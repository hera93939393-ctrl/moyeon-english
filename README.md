...
- 💬 **정보 안내 챗봇** — 버튼 클릭 또는 자유 질문. 커리큘럼·레벨테스트·시간표·셔틀버스·수강료·초등·중등·할인 8개 항목 중 원장이 파일을 올려둔 항목은 그 자료 내용으로, 안 올린 항목은 기본 안내 문구로 답변 (무료, API 비용 없음)
- 📎 **관리자 페이지에서 항목별 자료 업로드** — 8개 항목마다 PDF/텍스트 파일을 따로 등록·교체·삭제(기본값 복귀) 가능. 예: "시간표" 항목에 실제 시간표 파일을 올리면, 학부모가 시간표를 물었을 때 챗봇이 그 파일 내용을 찾아서 답변
- 📅 **레벨테스트 예약** — 달력 UI로 날짜·시간 선택, 정원 관리(일 30명 / 시간대별 2명)
- ✏️ **예약 변경·취소** — 본인 인증 토큰 방식
- 📱 **원장 문자 알림** — 예약 접수 시 SOLAPI로 즉시 SMS 발송
- 🔒 **관리자 화면** (`/admin.html`) — 위 항목별 자료 업로드 + 예약 목록·정원 현황 확인. 실제 예약자 개인정보(이름·연락처)가 노출되는 화면이라 로그인(Basic Auth)으로 보호되어 있고, 계정 정보는 공개 저장소에 포함하지 않습니다. 동작 화면은 시연 영상에서 확인하실 수 있습니다.
